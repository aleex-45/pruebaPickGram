...
public class WarehouseTest {
	...
	static class ShipmentsManager {
		...
		private ShipmentInfo findBestRoute(List<Warehouse> warehouses, Order order, BoxType box)
			...
			List<ShipmentInfo> possibleShippings = new ArrayList<>();

			String targetState = order.targetState;
		...
		private BoxType findBestBoxType(Order order) throws NoSuitableBoxException {
			...
			for (BoxType box : boxTypes) {

				Boolean available = true;

				int[] boxMeasures = { box.height, box.length, box.width };

				Arrays.sort(boxMeasures);

				if (temp.weight > box.maxWeight) {
					continue;
				}
				for (int i = 0; i < itemMeasures.length; i++) {
					if (itemMeasures[i] > boxMeasures[i]) {
						available = false;
					}
				}
			 ...
			 * Boolean available = true;
			 * 
			 * if(temp.weight > box.maxWeight) { available = false; } if(temp.length >
			 * box.length) { available = false; } if(temp.height > box.length) { available =
			 * false; } if(temp.width > box.width) { available = false; }
			 * 

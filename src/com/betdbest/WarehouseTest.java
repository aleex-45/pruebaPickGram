    ...
    private ShipmentInfo findBestRoute(List<Warehouse> warehouses, Order order, BoxType box)
    	...
    	for(Warehouse wh : warehouses) {
    		...
    		for(CarrierPricing carrierPricing : pricings) {
    			if(carrierPricing.getWarehouse().equals(wh) && carrierPricing.getTargetState().equals(targetState)) {
...
/*
    	for(BoxType box: boxTypes) {
    		...
    		if(temp.length > box.length) {
    			available = false;
    		}
    		if(temp.height > box.length) {
    			available = false;
    		}
    		if(temp.width > box.width) {
    			available = false;
    		}
